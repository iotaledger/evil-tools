...
type ParametersTool struct {
	...
	FaucetSplitNumber int      `default:"80" usage:"Number of outputs to split faucet outputs into, this determine number of txs that are sent per one faucet request. Reduce this number if not all txs are accepted due to congestion. "`
